...
authfile = "/home/remi/dev/igreedy-1.0/datasets/auth"
    ...
    def __init__(self, data, wait=True, sleep_notification=None, key=None, id=None):
        ...
        if data is not None:
            ...
            try:
                ...
                conn = urllib.request.urlopen(request, self.json_data.encode('utf-8'))
            ...
            while not enough:
                ...
                try:
                    ...
                    if meta["status"]["name"] == "Specified" or \
                        ...
                        raise InternalError(
                            "Internal error in #%s, unexpected status when querying the measurement fields: \"%s\"" % (
                            self.id, meta["status"]))
                    conn.close()
                except urllib.error.HTTPError as e:
